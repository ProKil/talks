...
function usePloy(number = 16) {
  ...
  function jumpPoints() {
    ...
    points.value = points.value.map((o) => {
        ...
        if (!closest) {
            // Handle the case where no points were found
            throw new Error('No points available to jump to');
            // Or provide a default value:
            // return [0, 0]; // or whatever makes sense as a default Range
        }
...
<template>
  ...
  >
    <div
      class="clip bg-gradient-to-r from-[#DC0082] to-white/10"
      :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
    />
    <div
      class="clip bg-gradient-to-l from-[#962fad] to-white/10"
